...
const Product = ({ product }) => {
    return (
        <Grid item lg={3} md={4} sm={6} xs={12}>
            <Paper sx={{ paddingBottom: "4px" }} elevation={4}>
                <Box className={style.productImage} sx={{ backgroundImage: "url(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSG_TB1R7WQQj3k8YOcqQ9zKnPYKfQyPMnj2w&usqp=CAU)" }}></Box>
                ...
                <Box paddingX={1} display={"flex"} justifyContent={"end"}>
                    <Rating onChange={(e, value) => console.log(value)} name="half-rating" defaultValue={2.3} precision={0.5} />
                </Box>
            </Paper>
        </Grid>
    )
